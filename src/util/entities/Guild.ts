...
export class Guild extends BaseClass {
    ...
    static async recalculateBoosts(guild_id: string) {
        const guild = await Guild.findOneOrFail({ where: { id: guild_id } });
        const nitroMembersCount = await Member.count({
            where: {
                guild_id,
                user: { premium: true },
            },
        ...
        guild.premium_subscription_count = nitroMembersCount * 30;

        if (guild.premium_subscription_count >= 14) guild.premium_tier = 3;
        else if (guild.premium_subscription_count >= 7) guild.premium_tier = 2;
        else if (guild.premium_subscription_count >= 2) guild.premium_tier = 1;
        else guild.premium_tier = 0;
